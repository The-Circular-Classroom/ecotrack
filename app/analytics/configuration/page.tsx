...
export default function AnalyticsConfigurationPage() {
  ...
  const fetchCatalog = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/assembly/products`);
  ...
  const fetchProductOptions = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/assembly/product-options`);
  ...
  useEffect(() => {
    ...
    fetch(`/api/assembly/item-types?schoolId=${selectedSchoolId}`)
      .then((res) => {
        if (!res.ok) {
          if (res.status === 401) {
  ...
  const handleUpdateProduct = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/assembly/products/${productId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  }, [fetchCatalog, productEditName, selectedCatalogProductId, showToast]);
  ...
  const handleUpdateProductType = useCallback(async () => {
    ...
    setUpdatingCatalog(true);
    try {
      const res = await fetch(`/api/assembly/product-types/${productTypeId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleUpdateStyle = useCallback(async () => {
    ...
    setUpdatingCatalog(true);
    try {
      const res = await fetch(`/api/assembly/styles/${styleId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleCreateProductType = useCallback(async () => {
    ...
    setUpdatingCatalog(true);
    try {
      const res = await fetch(`/api/assembly/product-types`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  }, [fetchProductOptions, newProductTypeName, showToast]);
  ...
  const handleCreateStyle = useCallback(async () => {
    ...
    setUpdatingCatalog(true);
    try {
      const res = await fetch(`/api/assembly/styles`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  }, [fetchProductOptions, newStyleName, showToast]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const endpoint = isEdit
        ? `/api/assembly/recipes/${editingRecipeId}`
        : `/api/assembly/recipes`;
  ...
  }, [editingRecipeId, fetchCatalog, mode, resetEditor, selectedStyleId, showToast, validateAndBuildPayload]);
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      if (deleteDialog.kind === 'product') {
        const res = await fetch(`/api/assembly/products/${deleteDialog.id}`, {
          method: 'DELETE',
        });
        ...
        const res = await fetch(`/api/assembly/product-types/${deleteDialog.id}`, {
          method: 'DELETE',
        });
        ...
        const res = await fetch(`/api/assembly/styles/${deleteDialog.id}`, {
          method: 'DELETE',
        });
        ...
        const res = await fetch(`/api/assembly/recipes/${deleteDialog.id}`, {
          method: 'DELETE',
        });
