...
export default function ItemDetailsModal({
  ...
  useEffect(() => {
    if (localSchoolId) onSchoolChange?.(localSchoolId);
  }, [localSchoolId]); // eslint-disable-line react-hooks/exhaustive-deps
              ...
              {isAdmin && (
                <div>
                  ...
                  {selectedSchool?.id ? (
                    ...
                    <select
                      value={localSchoolId}
                      onChange={(e) => dispatch({ type: 'PATCH', payload: { localSchoolId: e.target.value } })}
                      className={selectCls}
                    >
