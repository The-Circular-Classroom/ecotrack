...
export default function UploadCSVModal({ isOpen, onClose, selectedSchool }) {
  ...
  const validateAndSetFile = async (selectedFile) => {
    ...
    if (!allowedExtensions.includes(fileExt)) {
      setError('Invalid file format. Only Excel files (.xlsx, .xls) are allowed for the new template format.');
    ...
    try {
      ...
      const result = parseMatrixFile(workbook);
  ...
  const handleUpload = async () => {
    ...
    try {
      // Build final flat rows and convert to Excel
      ...
      const uploadFile = new File([blob], 'donation_upload.xlsx', { type: blob.type });
      ...
      const response = await fetch(`${apiUrl}/api/donation-drive/upload-csv`, {
        method: 'POST',
      ...
      setToast({
        open: true,
        message: `Upload successful! ${flatRows.length} transaction(s) submitted.`,
        severity: 'success',
      });
